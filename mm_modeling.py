...
from transformers import RobertaModel,BertweetTokenizer,RobertaConfig
__author__ = "Jianfei"
...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask):
        ...
        # This is actually dropping best_out entire tokens to attend to, which might
...
class TomBertForMMSequenceClassification(PreTrainedBertModel):
    ...
    def forward(self, input_ids, s2_input_ids, visual_embeds_att, gate, token_type_ids=None, s2_type_ids=None,
        ...
        transpose_img_embed = gate * s2_cross_output.unsqueeze(1)
...
class GateTomBert(PreTrainedBertModel):
    def __init__(self, config, num_labels=2, pooling="first"):
        super(TomBertForMMSequenceClassification, self).__init__(config)
        self.num_labels = num_labels
        self.pooling = pooling
        ...
        #self.ent2img_attention = BertCrossAttentionLayer(config)
        self.ent2img_pooler = BertPooler(config)
        self.gate_line = nn.Linear(config.hidden_size, 2)
        self.comb_attention = MultimodalEncoder(config)
        #self.comb_attention = BertLayer(config)
    ...
    def forward(self, input_ids, s2_input_ids, visual_embeds_att, gate, token_type_ids=None, s2_type_ids=None,
        ...
        s2_cross_output = self.ent2img_pooler(s2_cross_output_layer)
        #################s2_cross_output, _ = s2_cross_output_layer.max(1)
        s2_cross_output = s2_cross_output.unsqueeze(1)
        gate = nn.Softmax(dim = -1)(self.gate_line(s2_cross_output))
        gate = gate[:, :, 0]
        transpose_img_embed = gate * s2_cross_output
...
class TomBertweet(PreTrainedBertModel):
    """Entity-Attention BERT model for classification with text and image inputs, 1+text. (MBERT II)
    """
    def __init__(self, config, num_labels=2, pooling="first"):
        super(TomBertweet, self).__init__(config)
        self.num_labels = num_labels
        self.pooling = pooling
        self.bert = RobertaModel(config)
        self.s2_bert = RobertaModel(config)
        #self.s2_bert = self.bert
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.vismap2text = nn.Linear(2048, config.hidden_size)
...
class TomBertNoPoolingForMMSequenceClassification(PreTrainedBertModel):
    ...
    def forward(self, input_ids, s2_input_ids, visual_embeds_att, token_type_ids=None, s2_type_ids=None,
                attention_mask=None, s2_mask=None, added_attention_mask=None, labels=None, copy_flag=True):
        # Target-oriented Multimodal Sentiment Classification with BERT
        if copy_flag:
            print("*************************copy bert's weights to s2_bert******************")
            self.s2_bert = copy.deepcopy(self.bert)
